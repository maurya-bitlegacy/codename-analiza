...
namespace llvm {
  class ConstantAnalysis: public ForwardInterProceduralAnalysis<Function*, BasicBlock*, Constantv> {
     public:
      ...
      // void op(llvm::Value& i, ConstantState& state);
      // Constantv getConstantValueFor(llvm::Value* v, ConstantState& state);
      // Constantv evaluateCast(llvm::CastInst& castOp, ConstantState& state);
      // Constantv evaluateBinaryOperator(llvm::BinaryOperator& binOp, ConstantState& state);
