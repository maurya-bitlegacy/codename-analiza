...
Context<M, N, A> ForwardInterProceduralAnalysis<M, N, A>::initContext(M method, A entryv)
{
  ...
  {
    context->setValueBefore(cfg[i], topValue()); 
    context->setValueAfter(cfg[i], topValue()); 
  }
...
template class ForwardInterProceduralAnalysis<llvm::Function*, llvm::BasicBlock*, Constantv>;
