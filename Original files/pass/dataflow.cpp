...
namespace llvm {
  ...
  DataFlowResult DataFlow::run(Function &F, std::vector<void*> domain, BitVector boundaryCond, BitVector initCond) {
    ...
    *intVal = initCond;
    ...
    for(auto &BB: F) {
      if(std::find(boundaryBlocks.begin(), boundaryBlocks.end(), &BB) == boundaryBlocks.end()) {
      ...
      switch (direction) {
        case Direction::FORWARD:
          for(BasicBlock* Pred: predecessors(&BB))
            neighborList.push_back(Pred);
            break;

        case Direction::BACKWARD:
          for(BasicBlock* Succ: successors(&BB))
            neighborList.push_back(Succ);
          break;
    ...
    while (!converged) {
      ...
      for(BasicBlockList::iterator BB = traversalOrder.begin(), BE = traversalOrder.end(); BB != BE; ++BB) {
        ...
        if (!meetInputs.empty()) *blockInput = applyMeetOp(meetInputs);
